...
export type WeatherVars = {
  ...
  thunder?: boolean;
  advisory?: { gale?: boolean; thunder?: boolean };
};
...
export function judge(vars: WeatherVars, th: Thresholds): Judgment {
  ...
  const S = Math.round(
    windScore*0.35 + waveScore*0.25 + swellScore*0.2 + rainScore*0.1 + visScore*0.1
  );
  const label: Judgment["label"] = S >= 70 ? "OK" : S >= 40 ? "注意" : "中止";
  const reasons = [
